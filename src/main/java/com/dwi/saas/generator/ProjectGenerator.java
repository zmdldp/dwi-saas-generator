...
public class ProjectGenerator {
    ...
    public void build() {
        ...
        log.info("isChildModule：{}", isChildModule);
        // 创建项目跟文件夹
...
//            this.generatorApiModular(serviceName, objectMap);
            ...
            if(config.getIsBoot()) {
                ...
                System.err.println("请注意： 请手动 在saas-boot/pom.xml里面加入<module>, 在server模块依赖新增的Controller模块");
                return;
            }
    ...
    private void printCloudInfo(String serviceName) {
        ...
        System.err.println("------------------------------------------");
        System.err.println("将下面的配置手动加入nacos中 saas-zuul-server.yml");
        System.err.println(String.format("    %s:\n" +
                "      path: /%s/**\n" +
                "      serviceId: %s-server", config.getServiceName(), config.getServiceName(), serviceName));

        System.err.println("------------------------------------------");
        System.err.println("将下面的配置手动加入nacos中 saas-gateway-server.yml");
