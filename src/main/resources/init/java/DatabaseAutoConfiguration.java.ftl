 ...
 * 此时，项目的多租户模式切换成：${r"${saas.database.multiTenantType}"}。
 * <p>
 * NONE("非租户模式"): 不存在租户的概念
...
@ConditionalOnExpression("!'DATASOURCE'.equals('${r"${saas.database.multiTenantType}"}')")
public class ${service}DatabaseAutoConfiguration extends BaseMasterDatabaseConfiguration {
    ...
    public ${service}DatabaseAutoConfiguration(MybatisPlusProperties properties,
        ...
        log.debug("检测到 saas.database.multiTenantType!=DATASOURCE，加载了 ${service}DatabaseAutoConfiguration");
    }
